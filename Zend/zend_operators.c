...
static zend_always_inline zend_string* __zval_get_string_func(zval *op, zend_bool _try) /* {{{ */
{
try_again:
	switch (Z_TYPE_P(op)) {
		case IS_UNDEF:
		...
		case IS_ARRAY:
			zend_error(E_NOTICE, "Array to string conversion");
			return (_try && UNEXPECTED(EG(exception))) ?
		...
		case IS_OBJECT: {
			...
			if (Z_OBJ_HT_P(op)->cast_object) {
				...
				zval *z = Z_OBJ_HT_P(op)->get(op, &tmp);
				if (Z_TYPE_P(z) != IS_OBJECT) {
					zend_string *str = _try ? zval_try_get_string(z) : zval_get_string(z);
					zval_ptr_dtor(z);
					return str;
				}
				zval_ptr_dtor(z);
			...
			return _try ? NULL : ZSTR_EMPTY_ALLOC();
		}
		case IS_REFERENCE:
			op = Z_REFVAL_P(op);
			goto try_again;
